...
def _gdn_prefill_impl(q, k, v, state, A_log, a, dt_bias, b, cu_seqlens, scale, output, new_state):
    ...
    if output is None:
        output_tensor = torch.empty(
            (q.shape[0], num_v_heads, value_dim),
            dtype=torch.bfloat16,
            device=q.device,
        )
    else:
        output_tensor = _as_torch_tensor(output)

    if new_state is None:
        new_state_tensor = torch.empty(
            (num_seqs, num_v_heads, head_dim, value_dim),
            dtype=torch.float32,
            device=q.device,
        )
    else:
        new_state_tensor = _as_torch_tensor(new_state)
    ...
    # Keep BV >= 16: Triton tl.dot on the Modal/Blackwell stack fails to
    # compile state contractions whose N dimension is 8.
    if num_seqs >= 16:
    ...
    # `gdn_prefill_kernel_v2` currently hits a Triton compile failure on the
    # Modal/Blackwell stack for the K @ state^T contraction. Route all
    # non-flat workloads through the WY-based kernels, which exercise the same
    # operator family but compile reliably in this environment.
    use_split_wy = not use_flat_wy
    ...
    if use_split_wy:
        ...
        if use_precompute_u:
            ...
            wy_u = torch.empty(
                (num_seqs, num_v_heads, max_chunks, chunk, value_dim),
                dtype=torch.bfloat16,
                device=q.device,
            )
        else:
...
register_func("flashinfer.gdn_prefill", _gdn_prefill_impl, override=True)
gdn_prefill = _gdn_prefill_impl
...
def gdn_prefill_kernel_split_wy(
    ...
    for chunk_start in tl.range(seq_start, seq_end, CHUNK):
        ...
        state_tile = Gc * (state_tile + _dot_f32(tl.trans(x_chunk), K_tile_bf.to(tl.float32)))
...
def gdn_prefill_kernel_v2(
    ...
    for chunk_start in tl.range(seq_start, seq_end, CHUNK):
        ...
        state_tile_T = tl.trans(state_tile)
        state_k = _dot_f32(K_tile_bf.to(tl.float32), state_tile_T)              # [C, BV]

        Q_tile_bf = tl.load(q_ptr + qk_ptrs, mask=active[:, None], other=0.0)
        state_q = _dot_f32(Q_tile_bf.to(tl.float32), state_tile_T)             # [C, BV]
        ...
        state_update = _dot_f32(tl.trans(tail[:, None] * y_chunk), K_tile_bf.to(tl.float32))
